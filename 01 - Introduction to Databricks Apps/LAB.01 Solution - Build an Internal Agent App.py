...
# MAGIC %md
# MAGIC Create and configure a Databricks app based on the specified template:
# MAGIC
# MAGIC 1. In the [Apps tab of the compute page](/compute/apps), click **Create app**.
# MAGIC 1. Select **Create a custom app**.
# MAGIC 1. Specify the **App name**, using the name printed above, ensuring you observe the `-lab` suffix (to avoid interfering with the demo app) and click **Next**.
# MAGIC 1. Select **Add resource &gt; Serving endpoint**. Choose the following options:
# MAGIC    - For the serving endpoint, select *databricks-claude-3-7-sonnet*.
# MAGIC    - Set permission to *Can query*.
# MAGIC    - Set resource key to `serving-endpoint`.
# MAGIC 1. Click **Create app**.
# MAGIC 1. When the app is reach, click **Deploy**, and specify the path shown above in the **App code template** field.
# MAGIC
# MAGIC
...
# MAGIC %md
# MAGIC Follow the app source code link to open the folder containing the app source, and modify the files in that folder as needed to acheive the desired outcome.
# MAGIC
# MAGIC Refer to [app.yaml]($./lab_solution/app.yaml) and [app.py]($./lab_solution/app.py) for working examples.
