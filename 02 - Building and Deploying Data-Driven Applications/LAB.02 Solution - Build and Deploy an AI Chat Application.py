...
# MAGIC %md
# MAGIC Reconfigure the app resources to update the LLM and add the SQL warehouse:
# MAGIC
# MAGIC 1. In the [Apps tab of the compute page](/compute/apps), locate and select your app (the one created in the previous lab).
# MAGIC 1. In the top-right corner, click **Edit**.
# MAGIC 1. In the **Add metadata** screen, click **Next**.
# MAGIC 1. Change the **Serving endpoint** from *databricks-claude-3-7-sonnet* to *cookie_sales_agent_endpoint*.
# MAGIC 1. Add the SQL warehouse.
# MAGIC    - Click **Add resource**.
# MAGIC    - From the menu, select **SQL warehouse**.
# MAGIC    - For **SQL warehouse**, select *shared_warehouse*.
# MAGIC    - Copy the value for **Resource key** (`sql_warehouse`).
# MAGIC 1. Click **Save**.
