...
class DBAcademyHelper(NestedNamespace):
    ...
    @staticmethod
    def display_config_values(values):
        html = """<table style="width:100%">"""
        for desc, value in values:
            html += f"""
            <tr>
                <td style="white-space:nowrap; width:1em">{desc}:</td>
                <td><input type="text" value="{value}" style="width: 100%"></td></tr>"""
        html += "</table>"
        displayHTML(html)

    def display_config_add(self, key: str, desc: str = None):
        if not any(k == key for k,_ in self._config_display):
            self._config_display.append((key, desc or key))

    def display_config_remove(self, key: str):
        if not key:
            self._config_display = []
        else:
            self._config_display[:] = [p for p in self._config_display if p[0] != key]
    ...
    def __init__(self, **kwargs):
        ...
        if 'catalog_name' in self.dictionary:
            spark.sql(f'USE CATALOG {self.catalog_name}')
            self.display_config_add('catalog_name')

        if 'schema_name' in self.dictionary:
            spark.sql(f'USE SCHEMA {self.schema_name}')
            self.display_config_add('schema_name')

        try:
            initializers = getattr(self.__class__, '_initializers')
        ...
        if self._config_display:
            self.display_config_values(
                [(desc, self[key]) for key, desc in self._config_display]
            )
